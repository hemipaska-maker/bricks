...
class SequenceValidator:
    ...
    def validate(self, sequence: SequenceDefinition) -> list[str]:
        ...
        # Check 7: Empty sequence guard
        if not sequence.steps:
            errors.append(f"Sequence {sequence.name!r} has no steps")
        ...
        available_names = set(save_names) | set(sequence.inputs.keys())
        for key, ref in sequence.outputs_map.items():
